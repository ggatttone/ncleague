...
const TournamentDashboard = () => {
  ...
  const phases = useMemo(() => {
    const handlerKey = seasonWithMode?.tournament_modes?.handler_key as
  ...
  const handlePhaseAction = useCallback(
    (phaseId: string, action: 'schedule' | 'view' | 'close') => {
      switch (action) {
        case 'schedule':
          navigate(`/admin/schedule-generator?season=${selectedSeason}&phase=${phaseId}`);
          break;
        case 'view':
          navigate(`/admin/fixtures?season=${selectedSeason}&stage=${phaseId}`);
          break;
        case 'close':
    ...
    <AdminLayout>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          <Button variant="outline" onClick={() => setSimulatorOpen(true)} className="w-full">
            {t('pages.admin.tournamentDashboard.simulator.button')}
          </Button>
          <Button onClick={() => setClosePhaseOpen(true)} className="w-full">
            {t('pages.admin.tournamentDashboard.closePhase.button')}
          </Button>
        </div>
      </div>
