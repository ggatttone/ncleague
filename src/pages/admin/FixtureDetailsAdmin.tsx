...
import { MatchScoreEditor } from '@/components/admin/MatchScoreEditor';
...
const FixtureDetailsAdmin = () => {
  ...
  const matchQueryKey = ['match-admin', id] as const;
    ...
    <AdminLayout>
      <div className="max-w-3xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          ...
          <Button
            onClick={() => navigate(`/admin/fixtures/${id}/edit`)}
        ...
        {/* Card: Risultato Partita (punteggio inline editabile + marcatori) */}
        <div className="bg-card rounded-lg shadow p-6">
          <h2 className="text-xl font-bold mb-4">
            {t('pages.admin.fixtureDetails.resultSectionTitle')}
          </h2>
          ...
          <h3 className="text-base font-semibold mb-3">
            {t('pages.admin.fixtureDetails.scorersTitle')}
          </h3>
          {goalsLoading ? (
            ...
            <div className="space-y-2">
              {goals?.map((goal) => (
                <div key={goal.id} className="flex justify-between items-center">
                  <div>
                    ⚽ {goal.players.first_name} {goal.players.last_name} - {goal.minute}'
                  </div>
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        <AlertDialogTitle>
                          {t('pages.admin.fixtureDetails.deleteScorerDialogTitle')}
                        </AlertDialogTitle>
                        <AlertDialogDescription>
              ...
              {goals?.length === 0 && (
                <p className="text-muted-foreground text-sm">
                  {t('pages.admin.fixtureDetails.noScorers')}
                </p>
              )}
            </div>
          )}
