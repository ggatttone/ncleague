  ...
  return (
    <div>
      <h3 className="font-semibold mb-2">
        {t('pages.admin.tournamentDashboard.closePhase.step1.title')}
      </h3>
      <p className="text-sm text-muted-foreground mb-4">
        {t('pages.admin.tournamentDashboard.closePhase.step1.description')}
      </p>
      <ProactiveValidator standings={standings} />
      {!standings && (
        <p className="text-sm text-muted-foreground">
          {t('pages.admin.tournamentDashboard.closePhase.step1.noStandings')}
        </p>
      )}
    </div>
  );
...
const Step2OpenMatches = ({
  openMatches,
  overrides,
  setOverrides,
}: {
  openMatches: MatchWithTeams[];
  overrides: ManualOverrides;
  setOverrides: (o: ManualOverrides) => void;
}) => {
  const { t } = useTranslation();
  ...
  return (
    <div>
      <h3 className="font-semibold mb-2">
        {t('pages.admin.tournamentDashboard.closePhase.step2.title')}
      </h3>
      <p className="text-sm text-muted-foreground mb-4">
        {t('pages.admin.tournamentDashboard.closePhase.step2.description')}
      </p>
          ...
          <div key={match.id} className="p-4 border rounded-lg">
            ...
            >
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="cancel" id={`cancel-${match.id}`} />
                <Label htmlFor={`cancel-${match.id}`}>
                  {t('pages.admin.tournamentDashboard.closePhase.step2.cancel')}
                </Label>
              </div>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="freeze" id={`freeze-${match.id}`} />
                <Label htmlFor={`freeze-${match.id}`}>
                  {t('pages.admin.tournamentDashboard.closePhase.step2.freeze')}
                </Label>
              </div>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="result" id={`result-${match.id}`} />
                <Label htmlFor={`result-${match.id}`}>
                  {t('pages.admin.tournamentDashboard.closePhase.step2.enterResult')}
                </Label>
              </div>
            </RadioGroup>
            {overrides.resolved_matches?.[match.id]?.resolution === 'result' && (
              <div className="grid grid-cols-2 gap-2 mt-2">
                <Input
                  type="number"
                  placeholder={match.home_teams.name}
                  onChange={(e) => handleScoreChange(match.id, 'home', parseInt(e.target.value))}
                />
                <Input
                  type="number"
                  placeholder={match.away_teams.name}
...
const SortableTeamItem = ({ team }: { team: LeagueTableRow }) => {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className="p-2 border rounded bg-background flex items-center gap-2"
    >
...
const Step3Ties = ({
  ...
  ties: LeagueTableRow[][];
  setTies: Dispatch<SetStateAction<LeagueTableRow[][]>>;
}) => {
  const { t } = useTranslation();
  ...
  return (
    <div>
      <h3 className="font-semibold mb-2">
        {t('pages.admin.tournamentDashboard.closePhase.step3.title')}
      </h3>
      <p className="text-sm text-muted-foreground mb-4">
        {t('pages.admin.tournamentDashboard.closePhase.step3.description')}
      </p>
      <div className="space-y-4 max-h-96 overflow-y-auto p-1">
        {ties.map((group, index) => (
          <div key={index} className="p-4 border rounded-lg">
            <p className="font-medium mb-2">
              {t('pages.admin.tournamentDashboard.closePhase.step3.tieAtPoints', {
            ...
            >
              <SortableContext
                items={group.map((t) => t.team_id)}
                strategy={verticalListSortingStrategy}
              >
                <div className="space-y-2">
                  {group.map((team) => (
                    <SortableTeamItem key={team.team_id} team={team} />
                  ))}
                </div>
              </SortableContext>
            </DndContext>
...
const Step4Confirm = ({
  ...
  return (
    <div>
      <h3 className="font-semibold mb-2">
        {t('pages.admin.tournamentDashboard.closePhase.step4.title')}
      </h3>
      <p className="text-sm text-muted-foreground mb-4">
        {t('pages.admin.tournamentDashboard.closePhase.step4.description')}
      </p>
      <div className="space-y-4 max-h-96 overflow-y-auto p-1 text-sm">
        {Object.keys(overrides.resolved_matches).length > 0 && (
          <div>
            <h4 className="font-medium">
              {t('pages.admin.tournamentDashboard.closePhase.step4.resolvedMatches')}
            </h4>
            <ul className="list-disc pl-5 mt-1">
              {Object.entries(overrides.resolved_matches).map(([matchId, res]) => (
                <li key={matchId}>
                  Partita {matchId.substring(0, 8)}: {res.resolution}{' '}
                  {res.resolution === 'result' && `(${res.home_score}-${res.away_score})`}
                </li>
              ))}
            </ul>
          </div>
        ...
        {Object.keys(overrides.tie_breakers).length > 0 && (
          <div>
            <h4 className="font-medium">
              {t('pages.admin.tournamentDashboard.closePhase.step4.tieBreakers')}
            </h4>
            <ul className="list-disc pl-5 mt-1">
              {Object.entries(overrides.tie_breakers).map(([points, teamIds]) => (
                <li key={points}>
                  {t('pages.admin.tournamentDashboard.closePhase.step3.tieAtPoints', { points })}:{' '}
                  {teamIds.map((id) => teamsMap.get(id)).join(' > ')}
                </li>
              ))}
            </ul>
          </div>
        ...
        {Object.keys(overrides.resolved_matches).length === 0 &&
          Object.keys(overrides.tie_breakers).length === 0 && (
            <p className="text-muted-foreground">
              {t('pages.admin.tournamentDashboard.closePhase.step4.noOverrides')}
            </p>
          )}
      </div>
    </div>
...
export const ClosePhaseDialog = ({
  ...
  const { data: openMatches, isLoading: isLoadingMatches } = useSupabaseQuery<
    (Match & { home_teams: Team; away_teams: Team })[]
  >(
    ...
    () =>
      supabase
        .from('matches')
        .select(
          '*, home_teams:teams!matches_home_team_id_fkey(*), away_teams:teams!matches_away_team_id_fkey(*)',
        )
  ...
  const { data: teams } = useSupabaseQuery<Team[]>(['teams'], () =>
  ...
  const closePhaseMutation = useMutation({
    mutationFn: async (overrides: ManualOverrides) => {
      ...
      const { data, error } = await supabase.functions.invoke('tournament-phase-manager', {
        body: {
          season_id: seasonId,
          phase_to_close: currentPhaseKey ?? currentPhaseName,
  ...
  const steps = [
    ...
    {
      label: t('pages.admin.tournamentDashboard.closePhase.steps.openMatches'),
      disabled: !openMatches || openMatches.length === 0,
    },
    {
      label: t('pages.admin.tournamentDashboard.closePhase.steps.tieBreakers'),
      disabled: ties.length === 0,
    },
  ...
  const renderStepContent = () => {
    ...
    if (currentLabel === t('pages.admin.tournamentDashboard.closePhase.steps.summary'))
      return <Step1Summary standings={standings} />;
    if (currentLabel === t('pages.admin.tournamentDashboard.closePhase.steps.openMatches'))
      return (
        <Step2OpenMatches
          openMatches={openMatches || []}
          overrides={manualOverrides}
          setOverrides={setManualOverrides}
        />
      );
    if (currentLabel === t('pages.admin.tournamentDashboard.closePhase.steps.tieBreakers'))
      return <Step3Ties ties={orderedTies} setTies={setOrderedTies} />;
    if (currentLabel === t('pages.admin.tournamentDashboard.closePhase.steps.confirm'))
      return <Step4Confirm overrides={manualOverrides} teamsMap={teamsMap} />;
    return null;
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            {t('pages.admin.tournamentDashboard.closePhase.description', {
        ...
        <DialogFooter>
          ...
          {currentStep < steps.length - 1 ? (
            ...
            <Button
              onClick={() => closePhaseMutation.mutate(manualOverrides)}
              disabled={closePhaseMutation.isPending}
            >
